...
void Des_SetKey(const uint8_t Key[8]);// 设置密钥
static void F_func(bool In[32], const bool Ki[48]);// f 函数
static void S_func(bool Out[32], const bool In[48]);// S 盒代替
static void Transform(bool *Out, bool *In, const uint8_t *Table, int len);// 变换
static void Xor(bool *InA, const bool *InB, int len);// 异或
static void RotateL(bool *In, int len, int loop);// 循环左移
static void ByteToBit(bool *Out, const uint8_t *In, int bits);// 字节组转换成位组
static void BitToByte(uint8_t *Out, const bool *In, int bits);// 位组转换成字节组
//置换IP表
